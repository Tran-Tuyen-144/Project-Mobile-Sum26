...
class _CustomerMapScreenState extends State<CustomerMapScreen> {
  ...
  String _locationMessage = 'Đang dùng vị trí gần đúng để hiển thị kết quả.';
  ...
  Future<void> _requestLocation() async {
    ...
    try {
      if (!await Geolocator.isLocationServiceEnabled()) {
        _locationMessage =
            'Hãy bật dịch vụ vị trí để cập nhật khoảng cách chính xác.';
        return;
      }
      ...
      if (permission == LocationPermission.denied ||
        ...
        _locationMessage =
            'Bạn chưa cấp quyền vị trí; đang hiển thị các cơ sở gần nhất.';
        return;
      }
      final position = await Geolocator.getCurrentPosition();
      _locationMessage = 'Đã cập nhật vị trí của bạn trên bản đồ.';
      ...
      */
      _locationMessage = 'Không thể lấy GPS; đang hiển thị các cơ sở gần nhất.';
    } finally {
  ...
  Future<void> _openDirections(String address) async {
    ...
    if (!await launchUrl(uri, mode: LaunchMode.externalApplication) &&
        mounted) {
      _notice('Không thể mở Google Maps trên thiết bị này.');
    }
  }
  ...
  Future<void> _call(String phone) async {
    if (!await launchUrl(Uri(scheme: 'tel', path: phone.replaceAll(' ', ''))) &&
        mounted) {
      _notice('Không thể mở ứng dụng gọi điện.');
    }
  }
  ...
  Widget build(BuildContext context) => SingleChildScrollView(
    ...
    child: Column(
      ...
      children: [
        ...
        const Text(
          'Công viên, quán café, bãi cỏ và khu vui chơi thân thiện với thú cưng.',
  ...
  void _showPlace(PetFriendlyPlace place) => showModalBottomSheet(
    ...
    builder: (_) => _PlaceSheet(
      ...
      onReview: (review) async {
        ...
        await _savePlaces();
        if (mounted) _notice('Cảm ơn bạn đã đánh giá địa điểm.');
      },
    ),
  );
  void _showAddPlace() => showModalBottomSheet(
    ...
    builder: (_) => _AddPlaceSheet(
      onSave: (place) async {
        ...
        await _savePlaces();
        if (mounted) _notice('Đã thêm địa điểm Pet-friendly.');
      },
    ),
  );
}
    ...
    child: Stack(
      children: [
        ...
        const Positioned(
          ...
          bottom: 12,
          child: Text(
            'Chọn marker để xem đường đi và thời gian di chuyển',
  ...
  Widget build(BuildContext context) => _Sheet(
    child: Column(
      ...
      children: [
        ...
        _InfoGrid(
          items: [
            ('Khoảng cách', branch.distance),
            ('Di chuyển', branch.travelTime),
            ('Giờ mở cửa', branch.openTime),
            ('Trạng thái', 'Đang mở'),
            ('Đánh giá', '${branch.rating}/5'),
            ('Độ đông', branch.busyness),
          ],
        ),
      ...
      children: [
        ...
        _InfoGrid(
          items: [
            ...
            ('Đánh giá', '${place.rating.toStringAsFixed(1)}/5'),
            ('Bình luận', '${place.reviews.length}'),
          ],
        ),
        const SizedBox(height: 14),
  ...
  Future<void> _gps() async {
    ...
    try {
      ...
      if (p != LocationPermission.denied &&
        ...
        _address.text =
            'Vị trí GPS: ${pos.latitude.toStringAsFixed(5)}, ${pos.longitude.toStringAsFixed(5)}';
      }
    } catch (_) {
    } finally {
          ...
          child: Column(
            children: [
              ...
              TextFormField(
                controller: _image,
                decoration: const InputDecoration(
                  labelText: 'Hình ảnh (đường dẫn/URL, không bắt buộc)',
                ),
              ),
              const SizedBox(height: 16),
              SizedBox(
                ...
                child: FilledButton(
                  ...
                  child: const Text('Xác nhận thêm địa điểm'),
                ),
              ),
            ],
          ),
