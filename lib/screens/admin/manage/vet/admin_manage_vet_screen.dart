  ...
  Widget build(BuildContext context) {
    return DefaultTabController(
      ...
      child: Scaffold(
        ...
        appBar: AppBar(
          title: const Text('Bệnh viện Thú y', style: TextStyle(fontWeight: FontWeight.bold)),
          bottom: const TabBar(
...
class _VetServicesTabState extends State<_VetServicesTab> {
  final CollectionReference _servicesCollection = FirebaseFirestore.instance.collection('vet_services');
    ...
    return Scaffold(
      ...
      body: StreamBuilder<QuerySnapshot>(
        ...
        builder: (context, snapshot) {
          ...
          if (snapshot.hasData) {
            combined.addAll(snapshot.data!.docs.map((doc) => {...doc.data() as Map<String, dynamic>, 'id': doc.id, 'isFirebase': true}));
          }
      ...
      floatingActionButton: FloatingActionButton.extended(
        ...
        label: const Text("Thêm Dịch vụ"), icon: const Icon(Icons.add), backgroundColor: AppColors.primary,
      ),
    );
  }
        ...
        actions: [
          ...
          ElevatedButton(
            onPressed: () async {
              ...
              final data = {'name': nameCtrl.text.trim(), 'price': int.tryParse(priceCtrl.text) ?? 0, 'description': descCtrl.text.trim()};

              if (item != null) {
                item['isFirebase'] == true ? await _servicesCollection.doc(item['id']).update(data) : setState(() {
                  final idx = _mockServices.indexWhere((s) => s['id'] == item['id']);
                  if (idx != -1) _mockServices[idx] = {..._mockServices[idx], ...data};
                });
              } else {
...
class _VetDoctorsTabState extends State<_VetDoctorsTab> {
  final CollectionReference _doctorsCollection = FirebaseFirestore.instance.collection('vet_doctors');

  final List<Map<String, dynamic>> _mockDoctors = [
    {"id": "D1", "doctorId": "BS01", "name": "Nguyễn Văn A", "specialty": "Ngoại khoa", "phone": "0987654321", "email": "bs.a@pethub.com", "image": "", "isFirebase": false},
    {"id": "D2", "doctorId": "BS02", "name": "Trần Thị B", "specialty": "Nội khoa", "phone": "0123456789", "email": "bs.b@pethub.com", "image": "", "isFirebase": false},
  ];
  ...
  Widget build(BuildContext context) {
    return Scaffold(
                    ...
                    return Card(
                      ...
                      child: ListTile(
                        leading: CircleAvatar(
                          ...
                          child: (item['image'] == null || item['image'] == "") ? const Icon(Icons.person, color: AppColors.primary) : null,
                        ),
                        title: Text("${item['name']} (${item['doctorId']})", style: const TextStyle(fontWeight: FontWeight.bold)),
                        subtitle: Text("Chuyên khoa: ${item['specialty']}\nSĐT: ${item['phone']}"),
                        isThreeLine: true,
                        trailing: IconButton(
                            icon: const Icon(Icons.delete, color: Colors.red),
                            onPressed: () => item['isFirebase'] == true ? _doctorsCollection.doc(item['id']).delete() : setState(() => _mockDoctors.removeWhere((d) => d['id'] == item['id']))
                        ),
      ...
      floatingActionButton: FloatingActionButton.extended(
        ...
        label: const Text("Thêm Bác sĩ"), icon: const Icon(Icons.add), backgroundColor: AppColors.primary,
      ),
    );
  }
                ...
                GestureDetector(
                  onTap: () async {
                    ...
                    if (image != null) {
                      ...
                      setDialogState(() { currentImageUrl = CloudinaryUploadService.optimizedImageUrl(result.imageUrl); isUploading = false; });
                    }
                  },
                  child: CircleAvatar(
                    radius: 50, backgroundColor: Colors.grey[200],
                    backgroundImage: (currentImageUrl != null && currentImageUrl != "") ? NetworkImage(currentImageUrl!) : null,
                    child: isUploading ? const CircularProgressIndicator() : ((currentImageUrl == null || currentImageUrl == "") ? const Icon(Icons.camera_alt, color: Colors.grey, size: 30) : null),
                  ),
                ),
                const SizedBox(height: 15),
                Row(
                  children: [
                    Expanded(flex: 1, child: TextField(controller: idCtrl, decoration: const InputDecoration(labelText: 'Mã BS', border: OutlineInputBorder()))),
                    const SizedBox(width: 10),
                    Expanded(flex: 2, child: TextField(controller: nameCtrl, decoration: const InputDecoration(labelText: 'Họ & Tên (*)', border: OutlineInputBorder()))),
                  ],
                ),
                const SizedBox(height: 10),
                TextField(controller: specCtrl, decoration: const InputDecoration(labelText: 'Chuyên khoa', border: OutlineInputBorder())),
                const SizedBox(height: 10),
          ...
          actions: [
            ...
            ElevatedButton(
              onPressed: isUploading ? null : () async {
                if (nameCtrl.text.trim().isEmpty) {
                  ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Vui lòng nhập Tên bác sĩ!'), backgroundColor: Colors.orange));
                  return;
                }
                final data = {
                  'doctorId': idCtrl.text.trim(), 'name': nameCtrl.text.trim(),
                  'specialty': specCtrl.text.trim(), 'phone': phoneCtrl.text.trim(),
                  'email': emailCtrl.text.trim(), 'image': currentImageUrl
                };

                try {
                  if (item != null) {
                    item['isFirebase'] == true ? await _doctorsCollection.doc(item['id']).update(data) : setState(() {
                      final idx = _mockDoctors.indexWhere((d) => d['id'] == item['id']);
                      if (idx != -1) _mockDoctors[idx] = {..._mockDoctors[idx], ...data};
                    });
                  } else {
