...
class _AdminManageSpaScreenState extends State<AdminManageSpaScreen> {
  ...
  final CollectionReference _spaCollection = FirebaseFirestore.instance
                  ...
                  itemBuilder: (context, index) {
                    ...
                    return Card(
                      ...
                      child: ListTile(
                        ...
                        subtitle: Text(
                          "${formatCurrency(item['price'])}đ • ${item['category']}",
                        ),
                        trailing: IconButton(
  ...
  void _showSpaForm({Map<String, dynamic>? item}) {
    ...
    bool isCustomCategory =
        item != null && !_defaultCategories.contains(item['category']);
    final newCatCtrl = TextEditingController(
      text: isCustomCategory ? item['category'] : '',
    );

    String? currentImageUrl = item?['image'] ?? '';
    String selectedCat = isCustomCategory
        ? 'Khác'
        : (item == null ? _defaultCategories.first : item['category']);
              ...
              children: [
                ...
                TextField(
                  ...
                  decoration: const InputDecoration(
                    labelText: 'Tên dịch vụ (Bắt buộc)',
                ...
                if (selectedCat == 'Khác')
                  Padding(
                    ...
                    child: TextField(
                      ...
                      decoration: const InputDecoration(
                        labelText: 'Nhập tên danh mục mới',
                        border: OutlineInputBorder(),
                      ),
                    ),
                  ),
                      ...
                      try {
                        if (item != null) {
                          if (item['isFirebase'] == true) {
                            ...
                            setState(() {
                              final index = _mockServices.indexWhere(
                                (s) => s['id'] == item['id'],
                              );
                              if (index != -1) {
                                _mockServices[index] = {
