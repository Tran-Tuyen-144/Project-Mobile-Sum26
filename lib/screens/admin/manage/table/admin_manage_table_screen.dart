...
class _AdminManageTableScreenState extends State<AdminManageTableScreen> {
  ...
  final List<Map<String, dynamic>> _mockTables = [
    {"id": "T1", "name": "Bàn A1", "seats": 2, "seri": "SR-001", "isFirebase": false},
    {"id": "T2", "name": "Bàn A2", "seats": 2, "seri": "SR-002", "isFirebase": false},
    {"id": "T3", "name": "Bàn B1", "seats": 4, "seri": "SR-003", "isFirebase": false},
    {"id": "T4", "name": "Bàn B2", "seats": 4, "seri": "SR-004", "isFirebase": false},
    {"id": "T5", "name": "Bàn C1", "seats": 6, "seri": "SR-005", "isFirebase": false},
    {"id": "T6", "name": "Bàn C2", "seats": 6, "seri": "SR-006", "isFirebase": false},
  ];
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(title: const Text('Quản lý bàn', style: TextStyle(fontWeight: FontWeight.bold))),
                ...
                child: ListView.builder(
                  ...
                  itemBuilder: (context, index) {
                    ...
                    return Card(
                      ...
                      child: ListTile(
                        leading: Icon(isFirebase ? Icons.table_restaurant : Icons.table_bar, color: AppColors.primary, size: 30),
                        title: Text(table['name'], style: const TextStyle(fontWeight: FontWeight.bold)),
                        subtitle: Padding(
                          padding: const EdgeInsets.only(top: 4.0),
                          child: Text("Số chỗ: ${table['seats']}  •  Seri: $seri"),
                        ),
                        trailing: IconButton(
                            ...
                            onPressed: () {
                              if (isFirebase) {
                                ...
                                setState(() => _mockTables.removeWhere((t) => t['id'] == table['id']));
                              }
                            }
                        ),
        ...
        content: SingleChildScrollView(
          child: Column(
            ...
            children: [
              TextField(
                  controller: nameCtrl,
                  decoration: const InputDecoration(labelText: 'Tên bàn (Bắt buộc)', border: OutlineInputBorder())
              ),
              const SizedBox(height: 15),
              TextField(
                  controller: seatsCtrl,
                  keyboardType: TextInputType.number,
                  decoration: const InputDecoration(labelText: 'Số chỗ ngồi', border: OutlineInputBorder())
              ),
              const SizedBox(height: 15),
              TextField(
                  controller: seriCtrl,
                  decoration: const InputDecoration(labelText: 'Số Seri (Nội bộ)', border: OutlineInputBorder())
              ),
            ],
          ),
        ),
            ...
            onPressed: () async {
              ...
              if (newName.isEmpty) {
                ScaffoldMessenger.of(context).showSnackBar(
                    const SnackBar(content: Text('Vui lòng nhập Tên bàn!'), backgroundColor: Colors.orange)
                );
                return;
              }
              ...
              try {
                if (table != null) {
                  if (table['isFirebase'] == true) {
                    ...
                    setState(() {
                      final index = _mockTables.indexWhere((t) => t['id'] == table['id']);
                      if (index != -1) {
