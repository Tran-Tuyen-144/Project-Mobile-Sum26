...
class _VetItem {
  ...
  bool operator ==(Object other) =>
      ...
      other is _VetItem && runtimeType == other.runtimeType && id == other.id;
...
class _AdminVetServiceScreenState extends State<AdminVetServiceScreen> {
  ...
  String formatCurrency(dynamic price) => price.toString().replaceAllMapped(
  ...
  int get totalAmount {
    ...
    int extraPrice = _extraServices.fold(
      0,
                ...
                return Column(
                  children: [
                    DropdownButtonFormField<_VetItem>(
                      decoration: const InputDecoration(
                        border: OutlineInputBorder(),
                        hintText: 'Chọn dịch vụ chính',
                      ),
                      initialValue: _mainService,
                      items: combinedServices
                          .map(
                            (item) => DropdownMenuItem(
                              value: item,
                              child: Text(
                                "${item.name} - ${formatCurrency(item.price)}đ",
                              ),
                            ),
                          )
                          .toList(),
  ...
  void _navigateToCheckout() async {
    ...
    final result = await Navigator.push(
      context,
      MaterialPageRoute(
        builder: (_) => CheckoutVetScreen(orderData: orderData),
      ),
    );
