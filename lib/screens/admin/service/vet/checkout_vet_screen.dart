...
class CheckoutVetScreen extends StatefulWidget {
  ...
  orderData; // Chứa thông tin khách, pet, dịch vụ, tiền
    ...
    return Scaffold(
      ...
      body: ListView(
        ...
        children: [
          ...
          const Divider(),
          ListTile(
            title: const Text("Chủ nuôi"),
            trailing: Text(widget.orderData['customerName']),
          ),
          ListTile(
            title: const Text("Thú cưng"),
            trailing: Text(widget.orderData['petName'] ?? "Không tên"),
          ),
          ListTile(
            title: const Text("Dịch vụ chính"),
            trailing: Text(widget.orderData['mainService']),
          ),
          if ((widget.orderData['extraServices'] as List).isNotEmpty)
            ListTile(
              title: const Text("Dịch vụ kèm"),
              trailing: Text(
                (widget.orderData['extraServices'] as List).join(', '),
              ),
            ),
          const Divider(),
          Text(
            "TỔNG CỘNG: ${_money(widget.orderData['totalAmount'])}",
