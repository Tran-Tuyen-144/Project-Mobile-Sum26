  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: ListView(
        ...
        children: [
          const Text("THÔNG TIN DỊCH VỤ", style: TextStyle(fontWeight: FontWeight.bold, color: Colors.grey)),
          const Divider(),
          ListTile(title: const Text("Khách hàng"), trailing: Text(widget.customerName, style: const TextStyle(fontWeight: FontWeight.bold))),
          ListTile(title: const Text("Số điện thoại"), trailing: Text(widget.customerPhone)),
          ListTile(title: const Text("Dịch vụ"), trailing: Text(widget.serviceName, style: const TextStyle(fontWeight: FontWeight.bold))),
          const Divider(),
          Text("TỔNG TIỀN: ${_money(widget.totalAmount)}", style: const TextStyle(fontSize: 20, fontWeight: FontWeight.bold, color: AppColors.primary)),
      ...
      bottomNavigationBar: Padding(
        ...
        child: ElevatedButton(
          ...
          onPressed: () async {
            ...
            if (mounted) Navigator.pop(context, true); // Trả về true để reset màn hình Spa
          },
